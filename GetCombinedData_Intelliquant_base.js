...
function onDayClose(now){    
	...
	for (var i=0;  i<NumOfStocks; i++){
        if (DelistingDate_stock[i] >= today){ // 상폐일이 오늘 이후인 경우만 데이터 읽음
            ...
            share[i] = target_stock[i].getNoOfShare();
            logger.info(removeA(target_stock_code[i]) + ', O: ' + open[i] + ', H: ' + high[i] + ', L: ' + low[i] + ', C: ' + close[i] + ', V: ' + volume[i] + ', F: ' + volume_foreign[i] + ', I: ' + volume_institute[i] + ', R: ' + volume_retail[i] + ',  cap: ' + capital[i] + ', sh: ' + share[i]);
        }
    }   
